...
.. literalinclude:: ../../batchq/contrib/examples2/quickstart.py

and save it as ``job_submitter.py`` in ``MyFirstSubmission``. Note that
in the above code we use the second descriptor to initiate the first
descriptor in order to reuse the queue defined for ``desc1`` in
``desc2``. Go back to the shell and type:
...
.. note:: If choosing same input and output directory you will run into
   ...
   accidently resubmit a finished job. 

The above can be overcome by either separating ``input_directory`` and
``output_directory``, or by setting the submission id manually: 

.. literalinclude:: ../../batchq/contrib/examples2/quickstart2.py

To shorten the above you may use your previously defined
configurations

.. literalinclude:: ../../batchq/contrib/examples2/quickstart3.py

We can now generalise this to arbitrarily many jobs:

.. literalinclude:: ../../batchq/contrib/examples2/quickstart4.py
...
.. literalinclude:: ../../batchq/contrib/examples2/quickstart5.py
