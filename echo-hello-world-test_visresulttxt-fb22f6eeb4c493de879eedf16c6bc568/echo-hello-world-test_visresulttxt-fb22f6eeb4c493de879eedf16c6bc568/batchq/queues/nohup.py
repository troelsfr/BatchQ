...
#    input_paths = batch.Property([], display="Specify an input paths: ")
    ...
    ### TESTED AND WORKING
    pending = batch.Function(verbose=True, enduser=True, cache=5,idle_if=False) \
        .Qbool(False)

    ## TESTED AND WORKING
    submitted = batch.Function(create_workdir,verbose=True, enduser=True, cache=5, idle_if=True) \
        .Qcall(identifier_filename).Qjoin(_,".pid").isfile(_)

    lazy_finished = batch.Function(create_workdir,verbose=True, enduser=False, cache=5) \
        .Qjoin(identifier_filename,".finished").isfile(_).Qdo(2).Qbool(True).Qreturn() \
        .Qbool(False)

    finished = batch.Function(lazy_finished,verbose=True, enduser=True, cache=5, idle_if=False)

    lazy_running = batch.Function(lazy_finished,verbose=True, enduser=False, cache=5) \
        .Qdo(2).Qbool(False).Qreturn() \
        .Qjoin(identifier_filename,".running").isfile(_).Qdo(2).Qbool(True).Qreturn() \
        .Qbool(False)
       
    running = batch.Function(lazy_running,verbose=True, enduser=True, cache=5, idle_if=True) \
        .Qdon(2).Qbool(False).Qreturn() \
        .Qcall(pid).Qstore("pid").Qequal(_,"-1").Qdo(2).Qbool(False).Qreturn() \
        .Qget("pid").isrunning(_)

    failed = batch.Function(lazy_finished, enduser=True, cache=5,failed_if=True) \
        .Qdon(2).Qbool(False).Qreturn() \
